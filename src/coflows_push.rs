    ...
    async fn start(
        ...
        if !queues.contains_key(&flow_task.flow_id) {
                ...
                .await
            {
                ...
                if let Ok(res) = cl
                    .read_entry(&format!("flows:{}:default_dispatch_point", flow_endpoint))
                    .await
                {
                ...
                if let Ok(res) = cl
                    .read_entry(&format!(
                        ...
                        flow_endpoint, user_id, flow_task.flow_id
                    ))
                    .await
                {
                    dispatch_point = String::from_utf8_lossy(&res).to_string();
            ...
            if parallel_dispatch {
                queues.insert(flow_task.flow_id.clone(), (dispatch_point.clone(), None));
            } else {
                queues.insert(
                    flow_task.flow_id.clone(),
                    (dispatch_point.clone(), Some(VecDeque::new())),
                );
            }
        }
